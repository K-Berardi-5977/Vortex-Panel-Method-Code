...
for i = 1:NumPan %iterating over the ith control point
    for j = 1:NumPan %for each control point, iterate over j=1:n panels
        if j ~= i
            ...
            E = sqrt(B-A^2); %convenience variable
            Sj = S(j); %length of jth panel

            
            %compute normal geometric integral for each jth panel at the ith
            %control point
            K(i,j)= (Cn/2)*log((Sj^2 + 2*A*Sj + B)/B) + ((Dn - A*Cn)/E)*(atan2(Sj+A, E) ...
                -atan2(A, E)); 

            %compute tangent (surface) geometric interal for each jth panel at
