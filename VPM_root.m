function [Lift, S_tot, Circulation] = VPM_root(U, alphaD, c)

%============NUMERICAL VORTEX PANEL METHOD CODE============%

%========== Define Knowns ==========%
rho = 1.035; %density of air in kg/m^3
...
S_tot = sum(S); %total length of closed curve bounding the airfoil
...
alphaR = alphaD*(pi/180); %angle of attack converted to radians
...
%========== Circulation and Aerodynamic Loads ==========%
gamma_ds = gamma(:).*S(:); %sum of all the contributions to circulation from each panel
Circulation = sum(gamma(:).*S(:)); %CALCULATE NET CIRCULATION
...
[Nxx , Nyy, Vxy, rp, psi, THETA, Cpxy_mask] = PM_streamlines(XC, YC, XB, YB, phiR, S, gamma, U, alphaR, Cp, Nx, Ny, numPan);
...
plot(XC, Cp, 'k', 'HandleVisibility','off');
% plot(XC, Circulation, 'g-*')
...
% figure; hold on;
% plot(XC(1:half_x), Cl(1:half_x), 'r*');
% plot(XC(half_x+1:end), Cl(half_x+1:end), 'bo'); 
% plot(XC, Cl, 'k', 'HandleVisibility','off');
% title(['Lift Distribution on Airfoil Surface ($\alpha = ', num2str(alphaD), ')$'], 'Interpreter','latex');
% xlabel('X-Coordinate of Airfoil');
% ylabel('Lift Coefficient (Cl)');
% legend('Bottom Cl', 'Top Cl')
end